...
namespace Wordki.Test.Commands.SelectNextWordActionTests
{
    [TestFixture]
    public class SelectNextWordActionTest
    {

        SelectNextWordAction action;
        ...
        public void Select_next_word_if_selected_group_is_null()
        {
            ...
            action = new SelectNextWordAction(mockGroupSelectable.Object, mockWordSelectable.Object);
            action.Action();
        ...
        public void Select_next_word_if_selected_word_is_last()
        {
            IGroup group = Utility.GetGroup();
            mockGroupSelectable.Setup(x => x.SelectedGroup).Returns(group);
            mockWordSelectable.Setup(x => x.SelectedWord).Returns(group.Words.Last());
            action = new SelectNextWordAction(mockGroupSelectable.Object, mockWordSelectable.Object);
            action.Action();
            mockWordSelectable.VerifySet(x => x.SelectedWord = It.IsAny<IWord>(), Times.Never());
        }
